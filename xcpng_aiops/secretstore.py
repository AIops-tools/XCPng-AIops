"""Encrypted secret storage for xcpng-aiops.

Secrets (Xen Orchestra authentication tokens) are NEVER written to disk in plaintext. They
live in ``~/.xcpng-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``XCPNG_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "xcpng-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "XCPNG_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``XCPNG_<TARGET>_PASSWORD``; ``prefix='XCPNG_'`` and
