"""Policy engine — rule-based access control for xcpng MCP tools.

Rules are loaded from ``~/.xcpng-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("xcpng-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("XCPNG_POLICY_DISABLED") == "1":
        ...
        if not tiers:
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        f"No rules file at {self._path} — high/critical operations require a named approver (XCPNG_AUDIT_APPROVED_BY) by default. Author rules.yaml "
