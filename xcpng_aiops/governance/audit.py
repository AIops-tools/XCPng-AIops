"""Unified audit logging engine — all xcpng skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.xcpng-aiops/audit.db``.
...
_log = logging.getLogger("xcpng-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring XCPNG_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the XCPNG_AIOPS_HOME refactor
