...
class BudgetTracker:
    ...
    mid-session (e.g. an operator sets ``XCPNG_RUNAWAY_MAX`` higher) without a
    ...
    def _max_calls() -> int | None:
        return _env_int("XCPNG_MAX_TOOL_CALLS", None)

    @staticmethod
    def _max_seconds() -> int | None:
        return _env_int("XCPNG_MAX_TOOL_SECONDS", None)

    @staticmethod
    def _runaway_max() -> int:
        # 0 or negative disables the runaway breaker.
        return _env_int("XCPNG_RUNAWAY_MAX", _DEFAULT_RUNAWAY_MAX) or 0

    @staticmethod
    def _runaway_window() -> int:
        return _env_int("XCPNG_RUNAWAY_WINDOW_SEC", _DEFAULT_RUNAWAY_WINDOW_SEC) or 0
        ...
        with self._lock:
            ...
            if max_calls is not None and self._state.total_calls >= max_calls:
                raise BudgetExceeded(
                    ...
                    f"(XCPNG_MAX_TOOL_CALLS={max_calls}). Stop and summarize what "
            ...
            if max_seconds is not None and self._state.total_seconds >= max_seconds:
                raise BudgetExceeded(
                    ...
                    f"(XCPNG_MAX_TOOL_SECONDS={max_seconds}). Stop and report "
            ...
            if runaway_max > 0 and window > 0:
                ...
                if len(dq) >= runaway_max:
                    raise BudgetExceeded(
                        ...
                        f"(Raise XCPNG_RUNAWAY_MAX if this is genuinely intended.)",
                        rule="budget_runaway",
                    )
                dq.append(now)
